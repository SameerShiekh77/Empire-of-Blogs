...
def index(request):
    home_ad = HomePageAdPlacement.objects.filter(
        is_active=True, banner_placed_on="home_page"
    ).first()
...
def show_categories(request, unique_key=None):
    ...
    context = {"categories": categories, "navigation": navigation}
...
def store_detail(request, slug):
    ...
    navigation = BlogCategory.objects.all()[:5]
    faqs = FAQ.objects.filter(is_active=True, store=store).order_by("order")
    short_texts = None
    for faq in faqs:
...
def coupon_list(request, unique_key=None):
    ...
    if unique_key:
        ...
        return HttpResponseRedirect("/")
    coupons_ad = HomePageAdPlacement.objects.filter(
        is_active=True, banner_placed_on="coupon_page"
    ).first()
